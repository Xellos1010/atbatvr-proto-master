...
/// in a GvrPagedScrollRect based on the page's offset.
/// This creates a visual effect where the tiles will animate
/// in a staggered fashion relative to the page.
/// Requires the pages to have a GvrTiledPage script.
public class GvrTileScrollEffect : GvrBaseScrollEffect {
  public override void ApplyEffect(GvrBaseScrollEffect.UpdateData updateData) {
    GvrTiledPage tiledPage = updateData.page.GetComponent<GvrTiledPage>();

    if (tiledPage == null) {
      Debug.LogError("Page (" + updateData.page.name + ") does not have GvrTiledPage. " +
        "Cannot apply GvrTileScrollEffect.");
        return;
    }
