...
public class CustomScroll : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    #region public vars standard google native
    ...
    #region private vars google native
    ...
    #region public vars Custom Get Set google native
    ...
    public ChildrenPageProvider pageProviderSimple;

    public void SetActiveSlots(int iSlotstoActivate)
    {
        pageProviderSimple.VerifySlotsActive(iSlotstoActivate);
        
        /*for (int i = 0; i < pageCount; i++)
        {
            //pageProviderSimple.RemovePage(i);
            if (i > iSlotstoActivate)
            {
                if (pageProviderSimple == null)
                    pageProviderSimple = GetComponent<ChildrenPageProvider>();
                //make sure the page is a child
                pageProviderSimple.RemovePage(i);
                //pageProviderSimple.
            }
            else
            {
                if (pageProviderSimple == null)
                    pageProviderSimple = GetComponent<ChildrenPageProvider>();
                //make sure the page is a child
                pageProviderSimple.ReturnPage(i);
                //pageProviderSimple.
            }
        }*/
        //pageProviderSimple.SyncCacheActive();

        StartCoroutine(YieldSnapPage());
        
        /*if (iSlotstoActivate > 0)
            SnapToPage(0);*/
    }

    public IEnumerator YieldSnapPage()
    {
        yield return null;
        //SnapToPage(0);
    }

    public void SetActiveSlot(int iActiveSlot)
    {
        SnapToPage(iActiveSlot);
    }

    void Start()
    {
        #region Google Pagination System Init
        ...
        #region Evan Maximize Content Init
    ...
    void Update()
    {
        #region Google Pagination System Update
        if (!CanScroll)
        {
            return;
        }
    ...
    #region Google Pagination System functions
    ...
    #region Evan Custom Maximization System (TODO pull into new class)

    public void MaximizeElement(MaximizeContent content)
    {
        //Bring all elements of pages to be the new pages - resize all elements to fit panel
        //Set the parent of each page to an object pool Gameobject in parent (always Deactivated)
        //Reset the Pages variable
        //Set the active page to element
    }

    public void MinimizeElement(MaximizeContent content)
    {
        //Parent element page to Gameobject and have page call and layout all elements
