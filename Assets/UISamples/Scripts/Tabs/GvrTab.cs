...
public class GvrTab : MonoBehaviour {
  ...
  void OnValidate() {
    ...
    GvrTabGroup tabGroup = FindTabGroup();
    if (tabGroup == null) {
      Debug.LogError("Tab (" + gameObject.name + ")'s ToggleGroup must have a GvrTabGroup.");
    }
  }
  ...
  private void SetOpen(bool open) {
    ...
    if (open) {
      ...
      // Transition In
      IGvrUITransition transition = FindTransition();
      if (transition != null) {
  ...
  private GvrTabGroup FindTabGroup() {
    // The TabGroup is expected to be on the same object as the ToggleGroup.
    ToggleGroup toggleGroup = toggle.group;
    GvrTabGroup tabGroup = toggleGroup.GetComponent<GvrTabGroup>();
    return tabGroup;
  }

  private IGvrUITransition FindTransition() {
    return GetComponent<IGvrUITransition>();
  }
}
