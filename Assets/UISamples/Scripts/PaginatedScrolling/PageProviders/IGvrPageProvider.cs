...
/// for a GvrPagedScrollRect (Paginated Scrolling)
///
/// The derived class must inherit from MonoBehaviour and be placed on the
/// same object as GvrPagedScrollRect
///
/// Two generic implementations are included:
///
/// GvrChildrenPageProvider - This implementation automatically uses the children of the
/// GvrPagedScrollRect as the pages. The pages are in order of their SiblingIndex in the scene.
/// This is the simplest way to do PaginatedScrolling.
///
/// GvrPrefabPageProvider - This implementation takes a serialized list of prefabs that are
...
///
public interface IGvrPageProvider {
  /// Returns a float that represents the amount of space between pages
  /// in coordinates local to the GvrPagedScrollRect.
  float GetSpacing();
