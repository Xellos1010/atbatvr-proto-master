...
public class GvrPagedScrollRect : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler {
  ...
  private IGvrPageProvider pageProvider;

  /// Interface used to implement visual effect for scrolling this scroll rect.
  private GvrBaseScrollEffect[] scrollEffects;
  ...
  void Start() {
    pageProvider = GetComponent<IGvrPageProvider>();

    if (pageProvider == null) {
      throw new System.NullReferenceException(
        "GvrPagedScrollRect is missing an IGvrPageProvider. " +
        "Please look at IGvrPageProvider.cs for details.");
    }

    scrollEffects = GetComponents<GvrBaseScrollEffect>();
    if (scrollEffects.Length == 0) {
      Debug.LogWarning(
        "GvrPagedScrollRect does not have any GvrBaseScrollEffects. " +
        "Adding defaults.");
      gameObject.AddComponent<GvrTranslateScrollEffect>();
      gameObject.AddComponent<GvrFadeScrollEffect>();
      scrollEffects = GetComponents<GvrBaseScrollEffect>();
    }
  ...
  private void ApplyScrollEffects(RectTransform page) {
    ...
    GvrBaseScrollEffect.UpdateData updateData = new GvrBaseScrollEffect.UpdateData();
    ...
    foreach (GvrBaseScrollEffect scrollEffect in scrollEffects) {
